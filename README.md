...
The tool operates remotely from an attacker machine (e.g., Kali Linux), connects via SSH to the target, and performs a comprehensive audit based on industry-standard methodologies (GTFOBins).

## ⚡ Key Features
* **Remote Auditing:** Uses `paramiko` to manage secure SSH connections without requiring agent installation on the target.
* **SUID Scanner:** Scans the filesystem for SUID binaries and cross-references them with a known database of dangerous binaries (GTFOBins).
...
* **Target:** Ubuntu Server 22.04 (Hardened configuration with intentional vulnerabilities planted for testing).
...
```bash
git clone [https://github.com/your-username/escalation-hunter.git](https://github.com/your-username/escalation-hunter.git)
...
Below is an actual execution log from the test environment (v4.0):
